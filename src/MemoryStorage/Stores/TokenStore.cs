...
namespace OpenIddict.MemoryStorage.Stores
{
    public class TokenStore : IOpenIddictTokenStore<Token>
    {
        ...
        {
            if (Tokens == null && !string.IsNullOrEmpty(storageOptions.TokenFileStorage))
            {
                ...
                if (File.Exists(_storageFileName))
                {
                    ...
                    Tokens = JsonConvert.DeserializeObject<List<Token>>(content);
                }
                else
                {
                    Tokens = new();
        ...
        public ValueTask<Token> FindByIdAsync(string identifier, CancellationToken cancellationToken)
        {
            ...
            var app = Tokens.FirstOrDefault(x => x.Id == identifier);
        ...
        public ValueTask<Token> FindByReferenceIdAsync(string identifier, CancellationToken cancellationToken)
        {
            ...
            return ValueTask.FromResult(Tokens.Where(x => x.ReferenceId == identifier).FirstOrDefault());
        }
        ...
        public ValueTask UpdateAsync(Token token, CancellationToken cancellationToken)
        {
            ...
            var existingToken = Tokens.FirstOrDefault(x => x.Id == token.Id);
        ...
        private void SaveTokens()
        {
            ...
            {
                File.WriteAllText(_storageFileName, JsonConvert.SerializeObject(Tokens));
            }
        }
    }
}
